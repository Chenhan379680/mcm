...
def calculate_obscuration_time(params, uav_initial_pos, missile_initial_pos):
    ...
    for i, t in enumerate(times):
        cloud_center = calculate_cloud_center(uav_initial_pos, angle_degrees, uav_speed, t_release_delay, t_free_fall,
                                              t)
        missile_positon = calculate_missile_positon(missile_initial_pos, t)
...
def angle_between_vectors(a, b, in_degrees=True):
    # 点积与模长
    dot_prod = np.dot(a, b)
    ...
    cos_theta = np.clip(cos_theta, -1.0, 1.0)  # 防止数值误差超出 [-1,1]
...
def calculate_angles(uav_name, missile_name):
    uav = UAV_POSITIONS[uav_name]
    missile = MISSILE_POSITIONS[missile_name]
    MF = fake_target - missile
    MF_norm = np.linalg.norm(MF)

    def solve_for_S(v):
        ratio = 300 / v  # 导弹速 / UAV速

        # 方程: (t * |MF|) / |FY - (M1 + t*MF)| = ratio
        def equation(t):
            S = missile + t * MF
            return (t * MF_norm) / np.linalg.norm(uav - S) - ratio
    ...
    for v in [140, 70]:
        S = solve_for_S(v)
        vec_uav_to_s = S - uav
        angle = vector_angle(vec_uav_to_s)
        results[v] = angle
